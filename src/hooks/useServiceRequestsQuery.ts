...
function useServiceRequestsQuery(
  ...
  const queryFn = async (): Promise<ServiceRequestsResult> => {
    ...
    if (
      !includeArchived &&
      archivedStatusId &&
      filters.statusId === archivedStatusId
    ) {
      console.warn(
        "Query contradiction: Filtering by 'Archived' status while also excluding archived items.",
      );
      return { data: [], totalCount: 0 };
    }
    ...
    try {
      ...
      let countQueryBuilder = client
        .from("service_requests")
        .select("*", { count: "exact", head: true }); // Select can be minimal like 'id'

      // Apply standard filters from 'filters' object
      if (filters.locationId) {
        countQueryBuilder = countQueryBuilder.eq(
          "location_id",
          filters.locationId,
        );
      }
      ...
      if (filters.statusId) {
        ...
        countQueryBuilder = countQueryBuilder.not(
          "status_id",
          "eq",
          archivedStatusId,
        );
      ...
      throw new Error(
        "An unknown error occurred while fetching service requests.",
      );
    }
  };
...
export function useServiceRequestsByLocationId(locationId: string, 
  options: QueryOptions = {}, 
  statusMap: Record<string, string> | undefined,
  statusMapLoading: boolean) {
...
export function useServiceRequestsByServiceTypeId(serviceTypeId: string, 
  options: QueryOptions = {}, 
  statusMap: Record<string, string> | undefined,
  statusMapLoading: boolean) {
...
export function useServiceRequestsByStatusId(statusId: string, 
  options: QueryOptions = {},
  statusMap: Record<string, string> | undefined,
  statusMapLoading: boolean) {
